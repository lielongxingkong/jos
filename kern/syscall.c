...
sys_exofork(void)
{
	...
	ret = env_alloc(&new, curenv->env_id);
	if (ret < 0)
		return ret;
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	if (envid2env(envid, &env, 1) < 0)
		return -E_BAD_ENV;
	user_mem_assert(env, (void *)ROUNDUP(tf->tf_esp, PGSIZE) - PGSIZE, PGSIZE, PTE_P|PTE_U);
	env->env_tf = *tf;
	return 0;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (!(perm & (PTE_U|PTE_P)) || perm & ~PTE_SYSCALL || (uintptr_t)va >= UTOP)
		return -E_INVAL;
	if (envid2env(envid, &env, 1) < 0)
		return -E_BAD_ENV;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (env->env_ipc_recving == 1) {
		if ((uintptr_t)srcva < UTOP && env->env_ipc_dstva != NULL) {
			if ((r = sys_page_map(curenv->env_id, srcva, envid, env->env_ipc_dstva, perm)) < 0)
				return r;
...
sys_ipc_recv(void *dstva)
{
	...
	if ((uintptr_t)dstva < UTOP) {
		if ((r = sys_page_alloc(curenv->env_id, dstva, PTE_U|PTE_P)) < 0)
			return r;
